...
void drawCircleGrid() {
  for (int y = 0; y < rows*2; y++) {
    // Increase squareNoise for more staggering if needed
    ...
    for (int x = 0; x < cols; x++) {
      ...
      fill(grayValue);
      noStroke();
      // Calculate a random offset for each circle within a range to make them look staggered
      float offsetX = random(-squareSize, squareSize);
      float offsetY = random(-squareSize, squareSize);
      // Apply the offsets to the center position of each circle
      float adjustedX = x * squareSize + squareSize / 2 + offsetX;
      float adjustedY = y * squareSize + squareSize / 2 + offsetY;
      
      // Draw the circle at the adjusted position
      ellipse(adjustedX, adjustedY, circleRadius*2, circleRadius*2);
    }
  }
}
